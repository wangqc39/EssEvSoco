...
int32_t DecryptionConvertMp3(struct FileInfo *DistFp, uint32_t StartAddr, uint32_t FileLength)
{
    ...
    int32_t res;
    static int16_t WriteBuff[576 * 2];//576:22050采样率   576 * 2:44100采样率
...
/** 
 * [GetAudioMp3Format description]通过MP3文件头部的3字节，得到MP3文件的格式
 * @Author   tin39
 * @DateTime 2019年8月29日T9:56:15+0800
 * @param    StartAddr                [description]
 * @param    FileLength               [description]
 * @return                            [description]
 */
int32_t GetAudioMp3Format(uint32_t StartAddr, uint32_t FileLength, struct Mp3FormatInfo *Mp3Format)
{
    int32_t ret;
    uint8_t Mp3HeadBuff[8];
    
    Mp3Format->BitRate = 48;
    Mp3Format->SampleRate = 44100;
    ...
    //得到头部的8个字节
    DataFlashReadData(Mp3File.StartAddr, (uint8_t *)Mp3HeadBuff, 8);
    DecryptAndDeconfuse((uint8_t *)Mp3HeadBuff, 8);

    //通过MP3头部3字节，解析MP3的格式
    if(Mp3HeadBuff[0] != 0xFF)
        return 1;

    if(Mp3HeadBuff[1] == 0xFB && Mp3HeadBuff[2] == 0xB0)
    {
        Mp3Format->SampleRate = 44100;
        Mp3Format->BitRate = 192;
    }
    else if(Mp3HeadBuff[1] == 0xF3 && Mp3HeadBuff[2] == 0x60)
    {
        Mp3Format->SampleRate = 22050;
        Mp3Format->BitRate = 48;
    }
    else if(Mp3HeadBuff[1] == 0xFB && Mp3HeadBuff[2] == 0x70)
    {
        Mp3Format->SampleRate = 44100;
