...
unsigned int RemainFileLength;


/*const unsigned char AesKeyOriginTable[32] = 
{
  0x45, 0xf3, 0x22, 0x4d, 0x12, 0x1c, 0xfc, 0x74,
  0x95, 0x60, 0x0b, 0xac, 0x4f, 0x88, 0x8b, 0xad,
  0xea, 0x54, 0x39, 0x99, 0xa0, 0x28, 0xb7, 0xd9,
  0x93, 0x9a, 0x32, 0xbb, 0x67, 0xc3, 0x1c, 0x58,
};*/
unsigned char AESKeyTable[32];


//根据烧写文件的末8位计算出AES的key值
/*void InitAesKeyTable(unsigned char key)
{
	int i;
	for(i = 0;  i < 32; i++)
	{
		AESKeyTable[i] = AesKeyOriginTable[i] ^ key;
	}
}*/
...
void InvCipher( unsigned char * block, volatile unsigned char *expandedKey)
{
	unsigned char round = ROUNDS-1;
	expandedKey += BLOCKSIZE * ROUNDS;
	
	XORBytes( block, (uint8_t *)expandedKey, 16 );
	expandedKey -= BLOCKSIZE;
	
	do {
		InvShiftRows( block );
		InvSubBytesAndXOR( block, (uint8_t *)expandedKey, 16 );
		expandedKey -= BLOCKSIZE;
		InvMixColumns( block );
	} while( --round );
